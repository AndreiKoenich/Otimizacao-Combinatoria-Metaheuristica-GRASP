...
/* VERSÃO DE BUSCA COM VIZINHANÇA ESTOCÁSTICA */
...
/* Definição do número máximo da seed que será considerada, para geração de valores aleatórios. */
...
typedef struct
{
    /* Os três componentes abaixo são parâmetros de execução escolhidos pelo usuário. */
...
/* Estrutura para representar uma possível solução do problema de forma mais simplificada. */
typedef struct
{
...
A geração da vizinhança ocorre da seguinte maneira:
O primeiro vizinho é gerado substituindo o primeiro vértice selecionado pelo primeiro vértice não selecionado;
O segundo vizinho é gerado susbstituindo o primeiro e o segundo vértice selecionados pelo primeiro e segundo vértices não selecionados;
E assim por diante. O último vizinho será aquele que mais possuirá vértices do conjunto de vértices de L que não foram selecionados.

Ao final de cada geração, o conjunto contendo todos os vértices não selecionados é embaralhado para a próxima geração de vizinhos.
Dessa forma dizemos que a vizinhança é estocástica.
 */
void gera_vizinhos_estocasticos(Vizinhos vizinhos[], Instancia informacoes, int quantidade_vizinhos)
{
    ...
    for (int i = 0; i < quantidade_vizinhos; i++)
        for (int j = 0; j <= i; j++)
            vizinhos[i].vertices_selecionados[j] = informacoes.conjunto_L[informacoes.constante_l+j];
}
...
int examina_vizinhanca (Arestas solucao_parcial[],Instancia informacoes,float **matriz_adjacencia)
{
    ...
    gera_vizinhos_estocasticos(vizinhos,informacoes,quantidade_vizinhos); /* Gera e armazena todos os vizinhos. */
...
void grasp(float **matriz_adjacencia, Instancia informacoes, char nome_arquivo_entrada[], char nome_arquivo_saida[])
{
    ...
    FILE *arq_saida;
    if (!(arq_saida = fopen(nome_arquivo_saida,"w"))) /* Controle dos casos em que não e possivel abrir o arquivo de saída. */
    {
...
void leitura_arquivo(FILE *arq_entrada, char nome_arquivo_entrada[], char nome_arquivo_saida[]) /* Realiza a leitura de todas as linhas do arquivo de entrada. */
{
    ...
    printf("\nDigite um valor entre 0 e %d, para a seed da geracao de valores aleatorios:\n", MAXSEED-1);
    scanf("%d",&seed);
    srand(seed % MAXSEED);
    ...
    grasp(matriz_adjacencia, informacoes, nome_arquivo_entrada, nome_arquivo_saida);
...
void abre_arquivo(char nome_arquivo_entrada[], char nome_arquivo_saida[]) /* Abre e inicia a leitura do arquivo de entrada. */
{
    FILE *arq_entrada;

    if (!(arq_entrada = fopen(nome_arquivo_entrada,"r"))) /* Abertura do arquivo de entrada, com controle dos casos em que nao e possivel abrí-lo. */
    {
    ...
    leitura_arquivo(arq_entrada, nome_arquivo_entrada, nome_arquivo_saida); /* Realiza a indexação de todos os dados do arquivo de entrada, para depois realizar a busca. */
...
void inicia_programa()
{
    ...
    printf("\nVersao com vizinhanca estocastica\n");
    ...
    abre_arquivo(nome_arquivo_entrada,nome_arquivo_saida);
}
